...
public class Triangle {
    ...
    //!DIP violated, tightly-coupled to SSS
    public double area(SSSCalculator calculator) {
        //NOTE: Implementer in charged for compatibility
        if ( getA() != 0 && getB() != 0 && getC() != 0 )
            return Math.round((calculator.area(getA(), getB(), getC())) * 100.0) / 100.0;
        //!WRONG RESULT, incompatible inputs for algorithm
    ...
    //DRY for readability, comments

    //!DIP violated, tightly-coupled to SAS
    public double area(SASCalculator calculator) {
        //!WRONG RESULT, incompatible inputs for algorithm
        if ( getA() != 0 && getB() != 0 && getC() != 0 )
            return Math.round((calculator.area(getA(), getB(), getC())) * 100.0) / 100.0;
        //NOTE: Implementer in charged for compatibility
    ...
    //!DIP violated, tightly-coupled to SAS
    public double area(ASACalculator calculator) {
        //!WRONG RESULT, incompatible inputs for algorithm
        if ( getA() != 0 && getB() != 0 && getC() != 0 )
            return Math.round((calculator.area(getA(), getB(), getC())) * 100.0) / 100.0;
        //!WRONG RESULT, incompatible inputs for algorithm
